# Development Guidelines - OneAccess
...
│
├── oneaccess.php # Root plugin entrypoint.
...
- `npm run wp-env run cli -- --env-cwd=wp-content/plugins/oneaccess {YOUR_CMD_HERE}`: Run WP-CLI commands in the local environment.
- `npm run wp-env:test run cli -- --env-cwd=wp-content/plugins/oneaccess {YOUR_CMD_HERE}`: Run Composer/PHP tooling in the tests container.
...
Our specific ESLint configuration is defined in the [`eslint.config.mjs`](../eslint.config.mjs) file, which extends the default configuration provided by `@wordpress/eslint-plugin`.
...
OneAccess uses [Jest](https://jestjs.io/) for JavaScript/TypeScript unit testing, configured via [`jest.config.js`](../jest.config.js) and extending `@wordpress/scripts` defaults.
