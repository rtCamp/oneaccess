...
test.describe( 'plugin activation', () => {
	test( 'should activate and deactivate the plugin', async ( {
		...
		const dismissOnboardingModal = async () => {
			const modal = page.locator( '#oneaccess-site-selection-modal' );
			const backdrop = page.locator(
				'body.oneaccess-site-selection-modal'
			);
			...
			if ( await backdrop.isVisible() ) {
				await backdrop.evaluate( ( el ) => {
					el.classList.remove( 'oneaccess-site-selection-modal' );
				} );
			}
		};

		const pluginRow = page.locator(
			'tr[data-plugin="oneaccess/oneaccess.php"]'
		);
